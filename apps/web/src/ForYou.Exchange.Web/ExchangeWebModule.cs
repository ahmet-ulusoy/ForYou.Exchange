...
using ForYou.Exchange.AdministrationService.Web;
using ForYou.Exchange.IdentityService;
using ForYou.Exchange.IdentityService.Web;
using ForYou.Exchange.Localization;
using ForYou.Exchange.ProductService;
using ForYou.Exchange.ProductService.Web;
using ForYou.Exchange.SaasService;
using ForYou.Exchange.SaasService.Web;
using ForYou.Exchange.Shared.Hosting.AspNetCore;
using ForYou.Exchange.Web.Navigation;
using Polly;
using Prometheus;
using Serilog;
...
public class ExchangeWebModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(ExchangeWebModule).Assembly
            );
        });
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        {
            options.MenuContributors.Add(new ExchangeMenuContributor(configuration));
        });

        Configure<AbpToolbarOptions>(options =>
        {
            options.Contributors.Add(new ExchangeToolbarContributor());
        });
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<ProductServiceWebModule>(Path.Combine(
                    ...
                    $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}services{Path.DirectorySeparatorChar}product{Path.DirectorySeparatorChar}src{Path.DirectorySeparatorChar}ForYou.Exchange.ProductService.Web"));
            });
        }
